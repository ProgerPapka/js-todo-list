...
class TodoService {
    ...
    _createTodo(number, userId, title, description) {
        ...
        api.fetchUser(userId).then((res) => authorEl.append(document.createTextNode(`Автор: ${res.username}`)));
        authorEl.classList.add('card__author');

        const btnUpd = document.createElement('button');
        btnUpd.append(document.createTextNode('Изменить'));
    ...
    updateTodo(title, content, userId) {
        ...
        this.api.fetchUser(userId).then((res) => {
            this.card.getElementsByClassName('card__author')[0].innerText = `Автор: ${res.username}`
        });
    }
